...
public class LinuxDebuggerLocal_py extends DebuggerBase implements LinuxDebugger {
    ...
    // native methods
    private static PythonMethodInterface pmi;
    private  static void init0() {
        pmi = Frames.pmi;
    }
    private void attach0(int pid) {
        p_ps_prochandle = pid;
        long[] threadIds = pmi.getThreadsId();
        String[] names = pmi.getLibName();
        long[] bases = pmi.getLibBase();
    ...
    private long lookupByName0(String objectName, String symbol) {
        return pmi.lookUpByName(objectName, symbol);
    }
    ...
    private byte[] readBytesFromProcess0(long address, long numBytes) {
        byte[] result = null;
        try {
            //Frames.addressMap.put(address, address + numBytes);
            result = pmi.readBytesFromProcess(address, numBytes);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("read " + address + " exception");
        }
        return result;
    }
    private void attach0(String execName, String coreName) {}
...
//    public native static int  getAddressSize() ;
    ...
    interface WorkerThreadTask {
       public void doit(LinuxDebuggerLocal_py debugger) throws DebuggerException;
    }

    class LinuxDebuggerLocalWorkerThread extends Thread {
       LinuxDebuggerLocal_py debugger;
       WorkerThreadTask task;
       DebuggerException lastException;

       public LinuxDebuggerLocalWorkerThread(LinuxDebuggerLocal_py debugger) {
    ...
    public LinuxDebuggerLocal_py(MachineDescription machDesc,
    ...
    public synchronized boolean detach() {
        ...
        if (isCore) {
            ...
            class DetachTask implements WorkerThreadTask {
                ...
                public void doit(LinuxDebuggerLocal_py debugger) {
                    debugger.detach0();
