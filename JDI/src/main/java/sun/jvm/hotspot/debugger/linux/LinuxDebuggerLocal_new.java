...
public class LinuxDebuggerLocal_new extends DebuggerBase implements LinuxDebugger {
    ...
    private void attach0(int pid) {
        System.out.println(pid);
        //set p_ps_prochandle value
        p_ps_prochandle = pid;
        //set threadList and loadObjectList
        long[] threadIds = null;
        try {
            out.write("thread".getBytes());
            String res = in.readLine();
            String[] r = res.split(" ");
            threadIds = new long[r.length];
            for (int i = 0; i < threadIds.length; i++) {
                threadIds[i] = Long.parseLong(r[i].trim());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        for (long tid : threadIds) {
            threadList.add(getThreadForThreadId(tid));
        }
        String[] names = null;
        long[] bases = null;
        try {
            out.write("libname".getBytes());
            String res = in.readLine();
            names = res.split(" ");
            out.write("libbase".getBytes());
            String res2 = in.readLine();
            String[] r = res2.split(" ");
            bases = new long[r.length];
            for (int i = 0; i < bases.length; i++) {
                bases[i] = Long.parseLong(r[i].trim());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        for (int i = 0; i < names.length; i++) {
            loadObjectList.add(createLoadObject(names[i].trim(), 0, bases[i]));
        }
    }
    ...
    private long lookupByName0(String objectName, String symbol) {
        long addr = 0;
        //based on objectName and symbol, get address of symbol.
        try {
            out.write(("l_o_o_k_u_p " + objectName + " " + symbol).getBytes());
            String res = in.readLine();
            addr = Long.parseLong(res == null || res.equals("") ? "0": res.trim());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return addr;
    ...
    private ClosestSymbol lookupByAddress0(long address) {
        //symbol_for_pc
        String symbolName = null;
        long offset = 0;
        return createClosestSymbol(symbolName, offset);
    }
    //private native long[] getThreadIntegerRegisterSet0(int lwp_id) throws DebuggerException;
    private long[] getThreadIntegerRegisterSet0(int lwp_id) {
        return new long[28];
    }
    //private native byte[] readBytesFromProcess0(long address, long numBytes) throws DebuggerException;
    private byte[] readBytesFromProcess0(long address, long numBytes) {
        byte[] arr = null;
        try {
            out.write(("r_e_a_d " + address + " " + numBytes).getBytes());
            String res = in.readLine();
            while (res == null) {
                res = in.readLine();
            }
            String[] r = res.split(" ");
            arr = new byte[r.length];
            for (int i = 0; i < arr.length; i++) {
                arr[i] = (byte) Integer.parseInt(r[i].trim());
            }
        } catch (Exception e) {
            System.err.println("r_e_a_d " + address + " " + numBytes);
            e.printStackTrace();
        }
        return arr;
    }
    ...
    interface WorkerThreadTask {
       public void doit(LinuxDebuggerLocal_new debugger) throws DebuggerException;
    }

    class LinuxDebuggerLocalWorkerThread extends Thread {
       LinuxDebuggerLocal_new debugger;
       WorkerThreadTask task;
       DebuggerException lastException;

       public LinuxDebuggerLocalWorkerThread(LinuxDebuggerLocal_new debugger) {
         this.debugger = debugger;
         setDaemon(true);
       }

        //this method is called by start method of a new thread.
       public void run() {
        ...
        //this method is called by main thread.
    ...
    public LinuxDebuggerLocal_new(MachineDescription machDesc,
    ...
    public synchronized Address lookup(String objectName, String symbol) {
        ...
        if (isCore) {
            ...
            long addr = lookupByName0(objectName, symbol);
            Address result = (addr == 0 ? null : new LinuxAddress(this, handleGCC32ABI(addr, symbol)));
            return result;
        }
    }
    ...
    public synchronized long[] getThreadIntegerRegisterSet(int lwp_id)
        ...
        if (isCore) {
            ...
            class GetThreadIntegerRegisterSetTask implements WorkerThreadTask {
                int lwp_id;
                long[] result;
                public void doit(LinuxDebuggerLocal_new debugger) {
