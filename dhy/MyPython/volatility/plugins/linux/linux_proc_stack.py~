...
import volatility.plugins.linux.linux_proc_info as linux_proc_info
reload(linux_proc_info)
...
def yield_address(space, start, length = None, reverse = False):
    ...
    """
    if not length:
        length = linux_proc_info.address_size
    cont = True
...
#class linux_process_stack(linux_common.AbstractLinuxCommand):
class linux_process_stack(linux_proc_info.linux_proc_info):
    """
    Plugin to do analysis on the stack of user space applications.
    """

    def __init__(self, config, *args, **kwargs):
        linux_proc_info.linux_proc_info.__init__(self, config, *args, **kwargs)
    ...
    def calculate(self):
        ...
        #reload("linux_proc_info","linux_proc_info.py")
        for p in linux_proc_info.linux_proc_info.calculate(self):
            stats['tasks'] += 1
            if p:
    ...
    def analyze_stack(self, process_info, task, thread_number):
        ...
        @param process_info: The process info object
        ...
        address_size = linux_proc_info.address_size
        ...
        if not is_thread and entry_point:
            ...
            print "kai shi dizhi (ta shi stack bottom?,=start_stack)",stack_arguments
    ...
    def find_return_libc_start(self, proc_as, start_stack, return_start):
        ...
        for value in yield_address(proc_as, start_stack, reverse=True):
            if value == return_start:
                debug.info("Scanned {} stack addresses before finding the __libc_start_main return address".format((start_stack-address)/linux_proc_info.address_size))
                return address
            address -= linux_proc_info.address_size
                ...
                if not proc_as.is_valid_address(value):
                   invalid += 1
                    ...
                    if retval:
                        ...
                        return addr, retval
            addr -= linux_proc_info.address_size
    ...
    def calculate_annotations(self, frames):
        ...
        """
        size = linux_proc_info.address_size
        end = frames[-1].address
        start = frames[0].ebp_address
        l = linux_proc_info.read_int_list(start, end, frames[0].proc_as)
        result = {}
        offset = start
        for value in l:
            result[offset] = (value, "")
...
class stack_frame(object):
    ...
    def ret_address(self):
        return self.address - linux_proc_info.address_size
    ...
    def ebp_address(self):
        return self.address - (linux_proc_info.address_size * 2)

    @property
    def arg_address(self):
        return self.address - (linux_proc_info.address_size * 3)
    ...
    def get_locals(self):
        start = self.locals_end - linux_proc_info.address_size
        end = self.ret_address - linux_proc_info.address_size
        return linux_proc_info.read_int_list(start, end, self.proc_as)
